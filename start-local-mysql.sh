...
BINDIR=$(dirname $0)
source $BINDIR/pga_lord/envfile
...
echo "Create pga database ..."
$RUN_MYSQL_CMD mysql $MYSQL_OPTS -e "create database pga"
if [ $? -ne 0 ]; then
  echo "Failed to create database pga"
  exit 1
else
  echo "Database pga is created successfully"
fi
